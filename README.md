...
- **RFID is simulated.** Wokwi has no RC522/PN532 part. Three pushbuttons
  ...
  `PROTOCOL.md`.
- **The flame sensor has no native Wokwi part.** A slide switch labeled
  `FLAME` on GPIO35 is the fallback (see `firmware/diagram.json` and the
  wiring table below).
...
- **Wokwi part names used** (`wokwi-mq2-gas-sensor`, `wokwi-relay-module`,
  `wokwi-slide-switch`, `wokwi-potentiometer`, `wokwi-hc-sr04`, ...) and the
...
| GPIO35 | Flame sensor **fallback**: slide switch, pulled down 10k to GND | Flame detect - Cafeteria (GF-06) |
...
| Fire/safety set + indicators | 3-4 | MQ-2 + flame fallback switch (GF-06, LIVE) + 3 LEDs + buzzer (LIVE) |
...
| 4 | Raise MQ-2 high & sustained, or with flame | CONFIRMED EMERGENCY: all doors unlock, alarm, notification; acknowledge, drill/clear, doors return to locked. |
...
- Exact Wokwi part identifiers/pin names for `wokwi-mq2-gas-sensor`,
  `wokwi-relay-module`, `wokwi-slide-switch`, `wokwi-hc-sr04`,
  `wokwi-potentiometer`, and the ESP32 DevKit V1's own pin labels (`D2`,
  `3V3`, `GND.1`/`GND.2`/`GND.3`, `5V`) were not confirmed against a live
  Wokwi session - see the honesty note above and the Troubleshooting
  section for how to fix a mismatch quickly.
...
- Demo login passwords are intentionally simple and stored in plain text in
  ...
  system.
- Room floor-plan rectangles in `public/js/data/floorplan.js` are a clean
  schematic layout roughly proportional to Section 1's dimension table,
  not a scaled reproduction of the original AutoCAD drawing.

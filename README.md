...
Node.js "Central Smart Learning System" server turns its serial output into
a live, browser-based building-management dashboard.

```
smart-learning-center/
  firmware/    PlatformIO project - one ESP32, one diagram.json
  server/      Node.js central server (Express + ws), JSON-file storage
...
- **RFID is simulated.** Wokwi has no RC522/PN532 part. Three pushbuttons
  (`CARD1` = registered student, `CARD2` = registered faculty, `CARD3` =
  unregistered/invalid) plus a "virtual tap" panel on the website stand in
  for a real reader. This is labeled everywhere in the UI and in
  `PROTOCOL.md`.
- **Fire detection is smoke-only.** There is no flame sensor - the MQ-2 gas/
  smoke sensor on GF-06 (Cafeteria) is the only input to module D's five-level
  smoke scale.
...
- **Wokwi part names used** (`wokwi-mq2-gas-sensor`, `wokwi-relay-module`,
  `wokwi-potentiometer`, `wokwi-hc-sr04`, ...) and the
  exact ESP32 DevKit pin labels (`D2`, `3V3`, `GND.1`, `5V`, ...) are based
  on common Wokwi example projects at the time this was written and are
  believed correct, but **have not been verified against a live Wokwi
  session by the author of this repository**. If a wire shows red/broken
  when you first open `diagram.json` in the Wokwi editor, click the part to
  see its real pin names and adjust the `connections` array accordingly -
...
If you are wiring the Wokwi diagram incrementally (say, only the RFID
pushbuttons are connected so far), open the **Controls** page and switch off
every module you have not wired yet. A disabled module's `loop()` stops
running on the ESP32 entirely (see `firmware/src/module_flags.h`), so its
unwired/floating pins stop generating noisy readings, false alerts, and OLED
pages you don't care about yet - the simulation stays smooth and focused on
whatever you're actually testing. Toggling back on takes effect immediately,
no reflash needed.
...
|---|---|---|
| GPIO21 (SDA) / GPIO22 (SCL) | SSD1306 128x64 I2C OLED (`0x3C`) | Digital Information Display - Main Lobby |
| GPIO15 | DHT22 data | Temp/Humidity - Smart Classroom 1 (SF-03) |
| GPIO13 | PIR HC-SR501 `OUT` | Motion/presence - SF-03, also feeds the CCTV tile |
...
| GPIO25 | Pushbutton, `INPUT_PULLUP` | `CARD1` - registered student tap |
| GPIO26 | Pushbutton, `INPUT_PULLUP` | `CARD2` - registered faculty tap |
| GPIO27 | Pushbutton, `INPUT_PULLUP` | `CARD3` - unregistered/invalid card tap |
...
- **Server** (`server/*.js`, ES modules): `bridge.js` is the RFC2217 TCP
  client with Telnet/IAC stripping; `demoDevice.js` is a drop-in stand-in
  with the identical event interface for `BRIDGE_MODE=demo`; `db.js` is
  flat JSON-file persistence under `server/data/`; `server.js` wires it all
  into an Express REST API plus a `/ws` WebSocket relay, and also runs the
  light server-side simulation for virtual rooms/bins.
- **Dashboard** (`public/*`): no build step - static HTML/CSS and vanilla
  ES modules, hash-routed single page (`public/js/app.js`), Chart.js loaded
  from `server/node_modules` via a static route (works offline once
  `npm install` has run once).
...
- Click the part in the Wokwi editor and check its real pin names in the
  ...
  future update could rename `AOUT` on the MQ-2 part). Edit the matching
  entry in `firmware/diagram.json`'s `connections` array to match.
- If `wokwi-mq2-gas-sensor` or `wokwi-relay-module` are not available in
...
**Server can't find `chart.js`**
- Run `npm install` inside `server/` at least once; the dashboard's
  Environment page loads `/vendor/chartjs/chart.umd.js`, which Express
  serves straight out of `server/node_modules/chart.js/dist/`.
...
|---|---|---|
| Interactive smart boards | 4 | 1 relay-driven load (SF-03) LIVE; rest listed as building assets, SIMULATED |
| RFID readers | 3-4 + 10-15 tags | 1 reader simulated by 3 pushbuttons + web taps (LIVE); 12 tags seeded |
| CCTV / PIR | 5-6 | 1 PIR (SF-03, LIVE); other camera tiles SIMULATED |
...
| Digital displays | 3-4 | 1 SSD1306 OLED (Main Lobby, LIVE); others SIMULATED on dashboard |
| Central controller | 1-2 | 1 Node.js server (this repo) |
...
Also rendered live on the Diagnostics & Settings page.

| # | Action | Expected result |
|---|---|---|
| 1 | Tap `CARD1` | Attendance recorded, door unlocks for 3s. `CARD3` gives Access Denied, nothing recorded. |
| 2 | Start class now (SF-03) | Relay ON, room ACTIVE. Push DHT22 temp above 28C -> alert. End class -> data saved, relay OFF. |
| 3 | Raise MQ-2 briefly, then lower it | "Possible False Alarm" logged, no emergency. |
| 4 | Raise MQ-2 high & sustained | CONFIRMED EMERGENCY: all doors unlock, alarm, notification; acknowledge, drill/clear, doors return to locked. |
| 5 | Press the manual pull station | Immediate emergency path (bypasses D's verification window). |
...
- Exact Wokwi part identifiers/pin names for `wokwi-mq2-gas-sensor`,
  `wokwi-relay-module`, `wokwi-hc-sr04`, `wokwi-potentiometer`, and the
  ESP32 DevKit V1's own pin labels (`D2`, `3V3`, `GND.1`/`GND.2`/`GND.3`,
  `5V`) were not confirmed against a live Wokwi session - see the honesty
  note above and the Troubleshooting section for how to fix a mismatch
  quickly.
